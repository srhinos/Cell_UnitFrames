...
function Builder:CreateHealthFormatOptions(parent, widgetName)
    ...
    if const.HealthTextFormat.PERCENTAGE_ABSORBS_MERGED then
        table.insert(healthFormats,
            { "57% |cFFA7A7A7+" .. L["shields"], const.HealthTextFormat.PERCENTAGE_ABSORBS_MERGED })
    end
    ...
    if const.HealthTextFormat.NUMBER_ABSORBS_SHORT then
        table.insert(healthFormats,
            { F:FormatNumber(21377) .. "+" .. F:FormatNumber(16384) .. " |cFFA7A7A7+" .. L["shields"], const
                .HealthTextFormat.NUMBER_ABSORBS_SHORT })
    end
    if const.HealthTextFormat.NUMBER_ABSORBS_MERGED_SHORT then
        table.insert(healthFormats,
            { F:FormatNumber(21377 + 16384) .. " |cFFA7A7A7+" .. L["shields"], const.HealthTextFormat
                .NUMBER_ABSORBS_MERGED_SHORT })
    end
    ...
    if const.HealthTextFormat.CURRENT_SHORT_PERCENTAGE then
        table.insert(healthFormats,
            { F:FormatNumber(21377) .. " 32% |cFFA7A7A7HP", const.HealthTextFormat.CURRENT_SHORT_PERCENTAGE })
    end
    ...
    if const.HealthTextFormat.ABSORBS_ONLY_SHORT then
        table.insert(healthFormats,
            { F:FormatNumber(16384) .. " |cFFA7A7A7" .. L["shields"], const.HealthTextFormat.ABSORBS_ONLY_SHORT })
    end
